...
class MemoUpdate(generic.UpdateView):
    ...
    def get_form_kwargs(self):
        kwargs = super(MemoUpdate, self).get_form_kwargs()
        memo = Memo.objects.filter(pk=self.kwargs['pk']).get()
        kwargs['date'] = memo.date
        kwargs['seat'] = memo.seat
        kwargs['user'] = self.request.user
        kwargs['title'] = memo.title
        kwargs['text'] = memo.text
        return kwargs
